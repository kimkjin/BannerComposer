...
def draw_detections_on_image(image_bytes: bytes, fmt_config: dict) -> bytes:
    """Simula a composição final com a nova lógica de escala multi-restrição."""
    if not fmt_config:
    ...
    if 'composition_area' in rules and rule_type not in ['centered_logo']:
        ...
        scale_x_centering = max(
            target_x_on_canvas / focus_point_x if focus_point_x > 0 else 1.0,
            (canvas_w - target_x_on_canvas) / (img_w - focus_point_x) if (img_w - focus_point_x) > 0 else 1.0
        )
        scale_y_framing = 1.0
        if main_box:
            ...
            if person_box_h > 0 and comp_area_h > 0:
                scale_y_framing = comp_area_h / person_box_h
        
        scale_fill_canvas = max(canvas_w / img_w, canvas_h / img_h)
        ...
        resized_image = original_image.resize((new_w, new_h), Image.Resampling.LANCZOS)
        focus_x_scaled = int(focus_point_x * scale)
        focus_y_scaled = int(focus_point_y * scale)
        paste_x = int((canvas_w / 2) - focus_x_scaled)
        paste_y = int((canvas_h / 2) - focus_y_scaled)
    ...
    if 'logo_area' in rules:
        logo_area = rules['logo_area']; margin = rules.get('margin', {'x':0, 'y':0})
        if rule_type == 'centered_logo':
            lx1 = int((canvas_w - logo_area.get('width', 0)) / 2)
            ly1 = int((canvas_h - logo_area.get('height', 0)) / 2) # Centraliza a área de depuração também
        else:
            lx1 = margin.get('x', 0); ly1 = margin.get('y', 0)
        lx2 = lx1 + logo_area.get('width', 0); ly2 = ly1 + logo_area.get('height', 0)
        draw.rectangle([lx1, ly1, lx2, ly2], fill=COLOR_LOGO_AREA, outline=(0,0,0, 150), width=2)
        
    if 'composition_area' in rules:
        margin = rules.get('margin', {'x':0, 'y':20}); logo_area = rules.get('logo_area', {'width':0})
        if rule_type == 'centered_logo':
            cx1 = 0; cx2 = canvas_w
        else:
            cx1 = margin.get('x', 0) + logo_area.get('width', 0) + margin.get('y', 0)
            cx2 = canvas_w - margin.get('y', 0)
        cy1 = margin.get('y', 0); cy2 = canvas_h - margin.get('y', 0)
